...
class IncomeController extends GetxController {
  ...
  dataAPI(int pageNo) {
    fetchApi(url: "income/list", params: {
      'page': pageNo,
    ...
    return fetchApi(url: 'banking/getAllBank', get: true).then((value) {
      if (value != null) {
        ...
        for (var element in tmpList) {
          bankList.addIf(element['status'] == '1',
  ...
  Future<dynamic> getCat() {
    return fetchApi(url: 'category/type/Income', get: true).then((value) {
      if (value != null) {
        List tmpList = value['data'];
        for (var element in tmpList) {
          catList.addIf(element['status'] == '1',
